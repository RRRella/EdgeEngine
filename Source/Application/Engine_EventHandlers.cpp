...
void Engine::MainThread_HandleEvents()
{
	...
	mEventQueue_EdgineToWin_Main.SwapBuffers();
	std::queue<EventPtr_t>& q = mEventQueue_EdgineToWin_Main.GetBackContainer();
...
void Engine::SetMouseCaptureForWindow(HWND hwnd, bool bCaptureMouse)
{
...
void Engine::UpdateThread_HandleEvents()
{
	...
	while (!q.empty())
	{
		...
		case KEY_UP_EVENT:
		{
			...
			mInputStates.at(p->hwnd).UpdateKeyUp(static_cast<KeyCode>(p->wparam), p->bMouseEvent);
		} break;
...
void Engine::RenderThread_HandleWindowResizeEvent(const std::shared_ptr<IEvent>& pEvent)
{
	...
	mEventQueue_WinToEdgine_Update.AddItem(pResizeEvent);
...
void Engine::RenderThread_HandleToggleFullscreenEvent(const IEvent* pEvent)
{
	...
	if (bExclusiveFullscreenTransition)
	{
		...
		mEventQueue_EdgineToWin_Main.AddItem(std::make_shared< SetMouseCaptureEvent>(hwnd, bCapture, bVisible));
	}
	...
	else
	{
		...
		if(hwnd == mpWinMain->GetHWND())
			mEventQueue_EdgineToWin_Main.AddItem(std::make_shared< SetMouseCaptureEvent>(hwnd, bFullscreenStateToSet, bVisible));
	}
